...
__SQL
  ...
  if (!$sth->execute($user_id)) {
    my $brief_msg = "Lock user account failure (demote_user)"; 
    my $detail_msg = "Unable to lock a non-administrative user (user id = $user_id) who try to demote other user(s), please lock this guy manually ASAP.";
...
__SQL
  ...
  $subject = "Someone try to demote other user(s), check for it.";
  $content = "This guy, $user, who is not system administrator but some how get into user demotion function at $current_datetime from this IP address $ip_address. His/Her account has been locked. Try to find out what is going on.";
